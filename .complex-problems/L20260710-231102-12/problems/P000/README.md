...
- [ ] 审计并固定现有 KBLite、KB API、Context Packet 和 Echo 生成调用边界。
- [ ] 实现后端 revision、幂等 mutations、change feed 与兼容 snapshot。
- [ ] 实现 iOS 用户切换隔离、同步协调器和后端知识提取客户端。
- [ ] 将 Context Packet selected context 接入真实 Echo 生成并保留 query-based 本地降级。
- [ ] 补隐私、ownership、跨用户隔离、离线降级和重复注入测试。
- [ ] 运行后端全量测试、知识库 smoke、release QA、git diff check 和 iOS build。
- [ ] 更新实现/验收文档并关闭递归 ledger。
