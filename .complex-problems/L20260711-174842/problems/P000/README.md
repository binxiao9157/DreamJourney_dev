...
- [ ] 相同 entity ID 在不同账号、类型或内容下生成不同 cache key。
- [ ] 切换账号/登出后旧预热任务不能污染新 scope。
- [ ] 缓存并发读写不再直接访问无锁字典。
- [ ] graph/base/pending/outbox/sync history 都通过统一保护策略写入。
- [ ] 原有 envelope、路径和账号隔离行为兼容。
- [ ] 新 gates、release QA、Simulator/generic iPhoneOS build 与差异检查通过。
