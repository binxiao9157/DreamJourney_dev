...
- [ ] 先补跨账号授权策略单测和 middleware 集成失败用例。
- [ ] 实现 owner / family / recipient / system-only 策略评估。
- [ ] 接入安全响应头、shadow 日志和 enforce-safe 决策。
- [ ] 增加本地 HTTP smoke 和 release regression 可选 gate。
- [ ] 更新覆盖矩阵并完成后端、iOS 构建与 diff 验证。
- [ ] 关闭递归 ledger，记录生产 enforce 的剩余前置条件。
