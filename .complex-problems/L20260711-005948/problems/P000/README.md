...
- [ ] 实现并测试后端 v2 upsert/tombstone 与 change metadata。
- [ ] 实现并测试 iOS per-user base snapshot、三方合并和 delta 生成。
- [ ] 固化 local-only、删除、同实体双改、重复 operation 和 stale revision 行为。
- [ ] 增加部署态 smoke 与 release regression 可选 gate。
- [ ] 运行后端全量测试、iOS 构建、关键 smoke 和 `git diff --check`。
- [ ] 更新状态/部署文档，分别提交两仓库；未明确要求前不推送或部署。
