...
let points = FamilyFootprintTimeline.points(from: memories, ownerName: "陈家", includeDemoExpansion: true)
require(points.first?.year == 1962, "roadshow origin should sort before existing memories")
require(points.contains { $0.location == "绍兴老宅" }, "demo expansion should include family origin")
...
require(narrative.contains("绍兴老家"), "narrative should include family origin")
require(narrative.contains("江浙沪广"), "narrative should express current generation range")
require(narrative.contains("未来"), "narrative should reserve future generation area")

let journeySummary = FamilyFootprintTimeline.journeySummary(for: points, generation: .all)
require(journeySummary.routeText.contains("绍兴"), "journey summary should include the family origin")
require(journeySummary.routeText.contains("浙江"), "journey summary should include parent generation range")
require(journeySummary.routeText.contains("江浙沪广"), "journey summary should include current generation range")
require(journeySummary.routeText.contains("未来区域"), "journey summary should include future placeholder range")
require(journeySummary.detailText.contains("四代范围"), "journey summary should describe scripted generation ranges")
require(journeySummary.scaleText.contains("生活半径"), "journey summary should name expanding family radius")

let parentJourneySummary = FamilyFootprintTimeline.journeySummary(for: points, generation: .parents)
require(parentJourneySummary.routeText.contains("浙江"), "parent journey summary should use Zhejiang range")
require(parentJourneySummary.detailText.contains("省域"), "parent journey summary should reflect province-level lighting")

let currentJourneySummary = FamilyFootprintTimeline.journeySummary(for: points, generation: .current)
require(currentJourneySummary.routeText.contains("江苏"), "current journey summary should include Jiangsu")
require(currentJourneySummary.routeText.contains("广东"), "current journey summary should include Guangdong")

let nextJourneySummary = FamilyFootprintTimeline.journeySummary(for: points, generation: .next)
require(nextJourneySummary.routeText.contains("未来待点亮"), "next journey summary should avoid mock city endpoint")
