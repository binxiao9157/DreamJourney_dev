...
assertContains(appDelegate, "DJEnableArchiveRemoteFetch", "UIQA should expose an explicit launch flag for backend fetch verification")
assertContains(appDelegate, "FeatureFlagService.shared.set(.archiveRemoteFetch, enabled: true)", "UIQA backend fetch flag should enable the hidden feature inside the app")
