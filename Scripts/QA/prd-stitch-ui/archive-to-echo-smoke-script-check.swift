...
assertContains(featureFlags, "case .archiveToEchoSmoke:\n            return .login", "archive-to-echo smoke should establish its prevalidated QA session before Scene bootstrap")
...
assertContains(appDelegate, "hasSeededItem: Bool = false", "archive-to-echo smoke should defer seed creation until the account lease is available")
assertContains(appDelegate, "let didSeedItem = self.seedPendingArchiveAnalysisContext()", "archive-to-echo smoke should create its seed after root composition")
assertContains(appDelegate, "failureReason: \"archiveSeedUnavailable\"", "archive-to-echo smoke should report unavailable archive seed instead of timing out")
assertContains(appDelegate, "failureReason: \"missingSmokeDependencies\"", "archive-to-echo smoke should report missing root dependencies in its result artifact")
