...
assertContains(script, "LOCAL_BUNDLE_ID=\"${LOCAL_BUNDLE_ID:-com.yxj.dreamjourney.app}\"", "script must use the stable local simulator bundle override")
assertContains(script, "DREAMJOURNEY_PRODUCT_BUNDLE_IDENTIFIER=\"$LOCAL_BUNDLE_ID\"", "script must pass the local bundle override to Xcode")
...
assertContains(script, "\"familyManagementOnlyRowVisible\"[[:space:]]*:[[:space:]]*true", "script must validate publicly-ready familyManagement row")
assertContains(script, "\"familyManagementOnlyCanOpenSwitcher\"[[:space:]]*:[[:space:]]*true", "script must validate publicly-ready familyManagement route")
assertContains(script, "\"familySpaceCanOpenSwitcher\"[[:space:]]*:[[:space:]]*true", "script must validate publicly-ready familySpace route")
