...
require(echo.contains("tencentDigitalHumanContextSwitchReconnectDelay"), "Echo should delay reconnecting Tencent cloud render after persona context switches")
require(echo.contains("digitalHumanRuntimeRecoveryWorkItem"), "Echo should schedule bounded Tencent cloud-render recovery work")
require(echo.contains("shouldRecoverFromDigitalHumanRuntimeFailure"), "Echo should recover once from transient Tencent open failures")
require(echo.contains("scheduleCloudDigitalHumanRuntimeRecovery") && echo.contains("reason: \"contextDidChange\""), "persona switching should schedule a delayed Tencent session reconnect")
require(echo.contains("reason == \"tencent_cloud_open_failed\""), "Tencent open failure recovery should include generic SDK open failures")
