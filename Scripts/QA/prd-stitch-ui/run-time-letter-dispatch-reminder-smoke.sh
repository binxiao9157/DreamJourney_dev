...
grep -Eq '"inAppMessageCenterEntryTitle"[[:space:]]*:[[:space:]]*"8 条消息待处理 · 查看"' "$RESULT_FILE" || fail "Unified message center entry title should use total unread count across message kinds."
...
grep -Eq '"inAppMessageCenterUnreadCount"[[:space:]]*:[[:space:]]*8' "$RESULT_FILE" || fail "Unified message center should count time-letter, family invitation, care signal, and system notice unread messages before opening."
