...
echo "[archive-to-echo-smoke] Building UIQA app..."
xcodebuild \
...
echo "[archive-to-echo-smoke] Installing $BUNDLE_ID on $SIMULATOR_UDID..."
...
if grep -q "backend sync failed\\|无法连接服务器" "$RUNTIME_LOG" "$OS_LOG" 2>/dev/null; then
