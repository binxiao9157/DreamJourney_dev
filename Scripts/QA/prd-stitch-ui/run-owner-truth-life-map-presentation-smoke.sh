...
DERIVED_DATA_PATH="${DERIVED_DATA_PATH:-$ROOT_DIR/tmp/visual-qa/product-v4/DerivedDataOwnerTruthLifeMapPresentationSmoke}"
OUTPUT_ROOT="${OUTPUT_ROOT:-$ROOT_DIR/tmp/visual-qa/product-v4/owner-truth-life-map-presentation-smoke}"
RUN_ID="${RUN_ID:-$(date +%Y%m%d-%H%M%S)}"
OUTPUT_DIR="$OUTPUT_ROOT/$RUN_ID"
BUILD_LOG="$OUTPUT_DIR/build.log"
RUNTIME_LOG="$OUTPUT_DIR/runtime.log"
OS_LOG="$OUTPUT_DIR/oslog.log"
SCREENSHOT_PATH="$OUTPUT_DIR/01-owner-truth-life-map-presentation.png"
RESULT_COPY_PATH="$OUTPUT_DIR/owner-truth-life-map-presentation-smoke-result.json"
COMPLETION_PATTERN="OwnerTruthLifeMapPresentationSmoke completed"
...
fail() {
  printf '[owner-truth-life-map-presentation-smoke] %s\n' "$*" >&2
...
printf '[owner-truth-life-map-presentation-smoke] Building UIQA app...\n'
xcodebuild \
...
printf '[owner-truth-life-map-presentation-smoke] Installing %s on %s...\n' "$BUNDLE_ID" "$SIMULATOR_UDID"
...
RESULT_FILE="$DATA_CONTAINER/Documents/owner-truth-life-map-presentation-smoke-result.json"
rm -f "$RESULT_FILE"
...
printf '[owner-truth-life-map-presentation-smoke] Launching in-memory preview...\n'
xcrun simctl launch --console "$SIMULATOR_UDID" "$BUNDLE_ID" \
  DJUITestBypassLogin \
  DJRunOwnerTruthLifeMapPresentationSmoke > "$RUNTIME_LOG" 2>&1 &
CONSOLE_PID="$!"
...
grep -Eq '"completed"[[:space:]]*:[[:space:]]*true' "$RESULT_FILE" || fail "Life-map presentation smoke did not complete."
grep -Eq '"requestCount"[[:space:]]*:[[:space:]]*1' "$RESULT_FILE" || fail "Preview should read exactly once."
grep -Eq '"storyCount"[[:space:]]*:[[:space:]]*2' "$RESULT_FILE" || fail "Story aggregate drifted."
grep -Eq '"dimensionCount"[[:space:]]*:[[:space:]]*6' "$RESULT_FILE" || fail "Expected six stable dimensions."
grep -Eq '"privateFieldsRendered"[[:space:]]*:[[:space:]]*false' "$RESULT_FILE" || fail "Private fields must not render."
grep -Eq '"completionPercentageRendered"[[:space:]]*:[[:space:]]*false' "$RESULT_FILE" || fail "Completion percentage must not render."
...
grep -Eq '"DJRunOwnerTruthLifeMapPresentationSmoke"' "$RESULT_FILE" || fail "Life-map launch scenario drifted."

xcrun simctl io "$SIMULATOR_UDID" screenshot "$SCREENSHOT_PATH" >/dev/null
xcrun simctl terminate "$SIMULATOR_UDID" "$BUNDLE_ID" >/dev/null 2>&1 || true

printf '[owner-truth-life-map-presentation-smoke] Build log: %s\n' "$BUILD_LOG"
printf '[owner-truth-life-map-presentation-smoke] Runtime log: %s\n' "$RUNTIME_LOG"
printf '[owner-truth-life-map-presentation-smoke] OS log: %s\n' "$OS_LOG"
printf '[owner-truth-life-map-presentation-smoke] Result: %s\n' "$RESULT_COPY_PATH"
printf '[owner-truth-life-map-presentation-smoke] Screenshot: %s\n' "$SCREENSHOT_PATH"
