...
require(sdkBoundary.contains("let appKey: String"), "SDK configuration must carry backend-issued appKey")
require(sdkBoundary.contains("let accessToken: String"), "SDK configuration must carry backend-issued accessToken")
require(backendClient.contains("let appKey: String?"), "session credential must parse appKey")
require(backendClient.contains("let accessToken: String?"), "session credential must parse accessToken")
require(cloudRuntime.contains("missing_tencent_sdk_credential"), "cloud runtime must fail closed when backend omits Tencent SDK credentials")
require(cloudRuntime.contains("appKey: appKey"), "cloud runtime must pass appKey into SDK configuration")
require(cloudRuntime.contains("accessToken: accessToken"), "cloud runtime must pass accessToken into SDK configuration")
...
require(bridge.contains("chat(ChatParams"), "real bridge must map non-stream text drive")
