...
DERIVED_DATA_PATH="${DERIVED_DATA_PATH:-$ROOT_DIR/tmp/visual-qa/product-v4/DerivedDataOwnerTruthInterviewNaturalInputSmoke}"
OUTPUT_ROOT="${OUTPUT_ROOT:-$ROOT_DIR/tmp/visual-qa/product-v4/owner-truth-interview-natural-input-smoke}"
RUN_ID="${RUN_ID:-$(date +%Y%m%d-%H%M%S)}"
OUTPUT_DIR="$OUTPUT_ROOT/$RUN_ID"
BUILD_LOG="$OUTPUT_DIR/build.log"
RUNTIME_LOG="$OUTPUT_DIR/runtime.log"
OS_LOG="$OUTPUT_DIR/oslog.log"
SCREENSHOT_PATH="$OUTPUT_DIR/01-owner-truth-interview-natural-input.png"
RESULT_COPY_PATH="$OUTPUT_DIR/owner-truth-interview-natural-input-uiqa-result.json"
COMPLETION_PATTERN="OwnerTruthInterviewNaturalInputSmoke completed"
...
fail() {
  printf '[owner-truth-interview-natural-input-smoke] %s\n' "$*" >&2
...
printf '[owner-truth-interview-natural-input-smoke] Building UIQA app...\n'
xcodebuild \
...
printf '[owner-truth-interview-natural-input-smoke] Installing %s on %s...\n' "$BUNDLE_ID" "$SIMULATOR_UDID"
...
RESULT_FILE="$DATA_CONTAINER/Documents/owner-truth-interview-natural-input-uiqa-result.json"
rm -f "$RESULT_FILE"
...
printf '[owner-truth-interview-natural-input-smoke] Launching auto-run harness...\n'
xcrun simctl launch --console "$SIMULATOR_UDID" "$BUNDLE_ID" \
  DJUITestBypassLogin \
  DJEnableOwnerTruthCandidateReviewQA \
  DJRunOwnerTruthInterviewNaturalInputSmoke > "$RUNTIME_LOG" 2>&1 &
CONSOLE_PID="$!"
...
grep -Eq '"DJRunOwnerTruthInterviewNaturalInputSmoke"' "$RESULT_FILE" || fail "Natural-input launch scenario drifted."
grep -Eq '"DJEnableOwnerTruthCandidateReviewQA"' "$RESULT_FILE" || fail "QA gate launch argument drifted."

xcrun simctl io "$SIMULATOR_UDID" screenshot "$SCREENSHOT_PATH" >/dev/null
xcrun simctl terminate "$SIMULATOR_UDID" "$BUNDLE_ID" >/dev/null 2>&1 || true

printf '[owner-truth-interview-natural-input-smoke] Build log: %s\n' "$BUILD_LOG"
printf '[owner-truth-interview-natural-input-smoke] Runtime log: %s\n' "$RUNTIME_LOG"
printf '[owner-truth-interview-natural-input-smoke] OS log: %s\n' "$OS_LOG"
printf '[owner-truth-interview-natural-input-smoke] Result: %s\n' "$RESULT_COPY_PATH"
printf '[owner-truth-interview-natural-input-smoke] Screenshot: %s\n' "$SCREENSHOT_PATH"
