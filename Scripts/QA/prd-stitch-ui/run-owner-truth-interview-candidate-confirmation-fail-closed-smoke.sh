...
xcrun simctl launch --console "$SIMULATOR_UDID" "$BUNDLE_ID" \
  DJUITestBypassLogin \
  DJRunOwnerTruthInterviewCandidateConfirmationFailClosedSmoke > "$RUNTIME_LOG" 2>&1 &
CONSOLE_PID="$!"
...
grep -Eq '"failureDisposition"[[:space:]]*:[[:space:]]*"responseMismatch"' "$RESULT_FILE" || fail "Smoke must exercise the response mismatch boundary."
...
grep -Eq '"DJRunOwnerTruthInterviewCandidateConfirmationFailClosedSmoke"' "$RESULT_FILE" || fail "Launch scenario drifted."
