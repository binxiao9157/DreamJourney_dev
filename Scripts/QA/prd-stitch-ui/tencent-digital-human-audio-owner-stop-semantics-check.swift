...
require(providerSpeechResumeBody.contains("sendsGreeting: false,"), "Tencent TextOver should reopen Fire/Volcengine ASR capture without a Fire greeting")
require(providerSpeechResumeBody.contains("usesTurnScopedKnowledgeContext: true"), "Resumed Echo capture should preserve turn-scoped knowledge mode")
...
require(textOverCompletionBody.contains("\"resumeDelayMs\": Int(resumeDelay * 1_000)"), "Tencent TextOver logs must expose the computed resume delay for true-device diagnosis")
...
let blockedVoiceBody = functionBody(named: "handleBlockedRealtimeVoice", in: echo)
require(blockedVoiceBody.contains("providerCredentialBlocked"), "blocked realtime voice must record an explicit credential-boundary fallback")
...
require(cloudOpenBody.contains("event: \"duplicateOpenIgnored\""), "Tencent runtime must log duplicate open suppression for stream-in-progress diagnosis")
...
require(cloudInterruptBody.contains("event: \"providerInterruptIgnored\""), "Tencent runtime must ignore interrupt requests in ready/TextOver states instead of failing the route")
