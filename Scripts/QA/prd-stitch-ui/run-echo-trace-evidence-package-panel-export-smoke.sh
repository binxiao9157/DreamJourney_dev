...
require(result.get("personaBadgeName") == "UIQA 家人角色 · AI 数字分身", "Visible Echo persona badge text changed")
require(result.get("familyVoiceProfileBlocked") is True, "Family context must not select a cloned voice profile")
require(result.get("latestTurnIDHash") == correlation_hash("uiqa-panel-evidence-turn"), "Panel export latest turn hash changed")
require(result.get("latestProviderLogIdHash") == "missing", "Denied family voice must not call a synthesis provider")
...
require(result.get("latestRuntimeRoleVoiceSource") == "familyVoiceNotPermitted", "Panel export role voice source changed")
require(result.get("latestRuntimeVoiceProfileIdHash") == "missing", "Panel export must not retain a family voice profile")
...
require(runtime_diagnostics.get("roleVoiceSource") == "familyVoiceNotPermitted", "Exported role voice source changed")
require(
...
require("voiceProfileIdHash" not in runtime_diagnostics, "Exported runtime diagnostics must omit a denied family voice profile")
require(runtime_diagnostics.get("audioOwner") == "tencentDigitalHuman", "Exported audio owner changed")
voice_synthesis = packages[-1].get("voiceSynthesis", {})
require(voice_synthesis.get("status") == "unavailable", "Denied family voice should report unavailable synthesis")
require(voice_synthesis.get("failureReason") == "familyVoiceNotPermitted", "Denied family voice reason changed")
