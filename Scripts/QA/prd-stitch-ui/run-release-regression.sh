...
if [[ "$RUN_VOICE_CLONE_C1_C2_NON_DEVICE_GATE" == "1" ]]; then
  RUN_ID="$RUN_ID" \
  OUTPUT_ROOT="$OUTPUT_DIR/voice-clone-c1-c2-non-device-gate" \
  BACKEND_ROOT="$BACKEND_ROOT" \
  "$SCRIPT_DIR/run-voice-clone-c1-c2-non-device-gate.sh"
else
