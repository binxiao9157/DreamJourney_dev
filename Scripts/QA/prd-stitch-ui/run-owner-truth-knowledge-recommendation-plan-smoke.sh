...
grep -Eq '"singleRecommendationAccepted"[[:space:]]*:[[:space:]]*true' "$RESULT_FILE" || fail "Single server-planned breadth recommendation must be accepted."
grep -Eq '"emptyRecommendationAccepted"[[:space:]]*:[[:space:]]*true' "$RESULT_FILE" || fail "Empty eligible recommendation plan must be accepted."
grep -Eq '"nonReadyPlanAccepted"[[:space:]]*:[[:space:]]*true' "$RESULT_FILE" || fail "Unavailable recommendation plan must be accepted without coverage."
grep -Eq '"malformedPlanRejected"[[:space:]]*:[[:space:]]*true' "$RESULT_FILE" || fail "Malformed duplicate recommendation plan must fail closed."
grep -Eq '"useCasePhase"[[:space:]]*:[[:space:]]*"ready"' "$RESULT_FILE" || fail "Use case should publish a ready value-minimized plan."
grep -Eq '"useCaseSelectedCount"[[:space:]]*:[[:space:]]*2' "$RESULT_FILE" || fail "Use case should retain only the two safe recommendation slots."
grep -Eq '"useCaseCoverageDimensionCount"[[:space:]]*:[[:space:]]*6' "$RESULT_FILE" || fail "Use case should retain count-only coverage for six stable dimensions."
grep -Eq '"useCaseOpaqueIdentifiersExposed"[[:space:]]*:[[:space:]]*false' "$RESULT_FILE" || fail "Use case state must not retain Vault, candidate, template or memory identifiers."
grep -Eq '"useCaseStaleCompletionRejected"[[:space:]]*:[[:space:]]*true' "$RESULT_FILE" || fail "Stale account plan completion must fail closed."
