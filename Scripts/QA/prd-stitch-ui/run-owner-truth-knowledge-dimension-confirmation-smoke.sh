...
DERIVED_DATA_PATH="${DERIVED_DATA_PATH:-$ROOT_DIR/tmp/visual-qa/product-v4/DerivedDataOwnerTruthKnowledgeDimensionConfirmationSmoke}"
OUTPUT_ROOT="${OUTPUT_ROOT:-$ROOT_DIR/tmp/visual-qa/product-v4/owner-truth-knowledge-dimension-confirmation-smoke}"
RUN_ID="${RUN_ID:-$(date +%Y%m%d-%H%M%S)}"
OUTPUT_DIR="$OUTPUT_ROOT/$RUN_ID"
BUILD_LOG="$OUTPUT_DIR/build.log"
RUNTIME_LOG="$OUTPUT_DIR/runtime.log"
OS_LOG="$OUTPUT_DIR/oslog.log"
SCREENSHOT_PATH="$OUTPUT_DIR/01-owner-truth-knowledge-dimension-confirmation.png"
RESULT_COPY_PATH="$OUTPUT_DIR/owner-truth-knowledge-dimension-confirmation-uiqa-result.json"
COMPLETION_PATTERN="OwnerTruthKnowledgeDimensionConfirmationSmoke completed"
...
fail() {
  printf '[owner-truth-knowledge-dimension-confirmation-smoke] %s\n' "$*" >&2
...
printf '[owner-truth-knowledge-dimension-confirmation-smoke] Building UIQA app...\n'
xcodebuild \
...
printf '[owner-truth-knowledge-dimension-confirmation-smoke] Installing %s on %s...\n' "$BUNDLE_ID" "$SIMULATOR_UDID"
...
RESULT_FILE="$DATA_CONTAINER/Documents/owner-truth-knowledge-dimension-confirmation-uiqa-result.json"
rm -f "$RESULT_FILE"
...
printf '[owner-truth-knowledge-dimension-confirmation-smoke] Launching auto-run harness...\n'
xcrun simctl launch --console "$SIMULATOR_UDID" "$BUNDLE_ID" \
  DJUITestBypassLogin \
  DJEnableOwnerTruthCandidateReviewQA \
  DJRunOwnerTruthKnowledgeDimensionConfirmationSmoke > "$RUNTIME_LOG" 2>&1 &
CONSOLE_PID="$!"
...
grep -Eq '"completed"[[:space:]]*:[[:space:]]*true' "$RESULT_FILE" || fail "Confirmation smoke did not complete."
grep -Eq '"qaGateEnabled"[[:space:]]*:[[:space:]]*true' "$RESULT_FILE" || fail "QA gate should be enabled."
grep -Eq '"createdOutcome"[[:space:]]*:[[:space:]]*"created"' "$RESULT_FILE" || fail "Expected a created confirmation receipt."
grep -Eq '"replayOutcome"[[:space:]]*:[[:space:]]*"deduplicated"' "$RESULT_FILE" || fail "Expected an idempotent replay receipt."
grep -Eq '"confirmedDimension"[[:space:]]*:[[:space:]]*"keyDecisions"' "$RESULT_FILE" || fail "Expected keyDecisions confirmation."
grep -Eq '"authorityEpoch"[[:space:]]*:[[:space:]]*1' "$RESULT_FILE" || fail "Expected authority epoch on receipt."
...
printf '[owner-truth-knowledge-dimension-confirmation-smoke] Build log: %s\n' "$BUILD_LOG"
printf '[owner-truth-knowledge-dimension-confirmation-smoke] Runtime log: %s\n' "$RUNTIME_LOG"
printf '[owner-truth-knowledge-dimension-confirmation-smoke] OS log: %s\n' "$OS_LOG"
printf '[owner-truth-knowledge-dimension-confirmation-smoke] Result: %s\n' "$RESULT_COPY_PATH"
printf '[owner-truth-knowledge-dimension-confirmation-smoke] Screenshot: %s\n' "$SCREENSHOT_PATH"
