...
grep -Eq '"grantInviteRendered"[[:space:]]*:[[:space:]]*true' "$RESULT_FILE" || fail "Registered-account invitation control was not rendered."
grep -Eq '"grantRevokeRendered"[[:space:]]*:[[:space:]]*true' "$RESULT_FILE" || fail "Grant revoke control was not rendered."
