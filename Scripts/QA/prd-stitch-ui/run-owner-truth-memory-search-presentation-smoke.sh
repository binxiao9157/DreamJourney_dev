...
DERIVED_DATA_PATH="${DERIVED_DATA_PATH:-$ROOT_DIR/tmp/visual-qa/product-v4/DerivedDataOwnerTruthMemorySearchPresentationSmoke}"
OUTPUT_ROOT="${OUTPUT_ROOT:-$ROOT_DIR/tmp/visual-qa/product-v4/owner-truth-memory-search-presentation-smoke}"
RUN_ID="${RUN_ID:-$(date +%Y%m%d-%H%M%S)}"
OUTPUT_DIR="$OUTPUT_ROOT/$RUN_ID"
BUILD_LOG="$OUTPUT_DIR/build.log"
RUNTIME_LOG="$OUTPUT_DIR/runtime.log"
OS_LOG="$OUTPUT_DIR/oslog.log"
SCREENSHOT_PATH="$OUTPUT_DIR/01-owner-truth-memory-search-presentation.png"
RESULT_COPY_PATH="$OUTPUT_DIR/owner-truth-memory-search-presentation-smoke-result.json"
COMPLETION_PATTERN="OwnerTruthMemorySearchPresentationSmoke completed"
...
fail() {
  printf '[owner-truth-memory-search-presentation-smoke] %s\n' "$*" >&2
...
printf '[owner-truth-memory-search-presentation-smoke] Building UIQA app...\n'
xcodebuild \
...
printf '[owner-truth-memory-search-presentation-smoke] Installing %s on %s...\n' "$BUNDLE_ID" "$SIMULATOR_UDID"
...
RESULT_FILE="$DATA_CONTAINER/Documents/owner-truth-memory-search-presentation-smoke-result.json"
rm -f "$RESULT_FILE"
...
printf '[owner-truth-memory-search-presentation-smoke] Launching in-memory preview...\n'
xcrun simctl launch --console "$SIMULATOR_UDID" "$BUNDLE_ID" \
  DJUITestBypassLogin \
  DJRunOwnerTruthMemorySearchPresentationSmoke > "$RUNTIME_LOG" 2>&1 &
CONSOLE_PID="$!"
...
grep -Eq '"completed"[[:space:]]*:[[:space:]]*true' "$RESULT_FILE" || fail "Memory-search presentation smoke did not complete."
grep -Eq '"requestCount"[[:space:]]*:[[:space:]]*1' "$RESULT_FILE" || fail "Preview should read exactly once."
grep -Eq '"resultCount"[[:space:]]*:[[:space:]]*1' "$RESULT_FILE" || fail "Expected one bounded result."
...
grep -Eq '"DJRunOwnerTruthMemorySearchPresentationSmoke"' "$RESULT_FILE" || fail "Memory-search launch scenario drifted."

xcrun simctl io "$SIMULATOR_UDID" screenshot "$SCREENSHOT_PATH" >/dev/null
xcrun simctl terminate "$SIMULATOR_UDID" "$BUNDLE_ID" >/dev/null 2>&1 || true

printf '[owner-truth-memory-search-presentation-smoke] Build log: %s\n' "$BUILD_LOG"
printf '[owner-truth-memory-search-presentation-smoke] Runtime log: %s\n' "$RUNTIME_LOG"
printf '[owner-truth-memory-search-presentation-smoke] OS log: %s\n' "$OS_LOG"
printf '[owner-truth-memory-search-presentation-smoke] Result: %s\n' "$RESULT_COPY_PATH"
printf '[owner-truth-memory-search-presentation-smoke] Screenshot: %s\n' "$SCREENSHOT_PATH"
