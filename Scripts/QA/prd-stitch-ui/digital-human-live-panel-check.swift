...
assertContains(echo, "private func handleTencentDigitalHumanTextOverTimeout(", "Echo should recover if Tencent TextOver never arrives")
assertContains(echo, "reason: \"textOverTimeout\"", "Tencent TextOver timeout recovery should reject stale lifecycle callbacks")
...
assertContains(dialogEngine, "sendsGreeting: Bool = true,", "DialogEngine should support resumed listening without a greeting after Tencent provider speech")
assertContains(dialogEngine, "usesTurnScopedKnowledgeContext: Bool = false", "DialogEngine should preserve a default for non-Echo callers")
