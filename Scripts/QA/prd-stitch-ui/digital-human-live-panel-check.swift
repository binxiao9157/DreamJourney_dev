...
assertNotContains(defaultEnabledBlock, ".digitalHumanLivePanel", "Digital human live panel must stay hidden in the V4 Closed Pilot")
assertContains(featureFlags, ".digitalHumanLivePanel,", "Digital human live panel must remain a non-persistent QA capability")

assertContains(echo, "DigitalHumanLivePanelView", "Echo should own the live panel view")
assertContains(echo, "FeatureFlagService.shared.isEnabled(.digitalHumanLivePanel)", "Echo should gate the digital human panel through the feature flag")
...
assertContains(echo, "DJDisableDigitalHumanLivePanel", "Echo should keep an explicit QA troubleshooting launch argument")
...
assertContains(echo, "private var isDigitalHumanQAOverrideEnabled: Bool", "Echo should isolate digital-human launch arguments behind a QA-only helper")
assertContains(echo, "#if DEBUG || UI_QA_SIMULATOR", "Echo digital-human launch arguments must be compile-time restricted")
...
assertContains(echo, "correlations: [\"turn\":", "Tencent digital-human diagnostics should include a redacted Echo turn correlation")
assertContains(echo, "correlations: [\"request\":", "Tencent digital-human diagnostics should include a redacted provider request correlation")
assertNotContains(echo, "turnID=", "Tencent digital-human diagnostics must not print a raw Echo turn id")
assertNotContains(echo, "requestID=", "Tencent digital-human diagnostics must not print a raw provider request id")
...
assertContains(echo, "scheduleTencentDigitalHumanTextOverTimeout(", "Echo should schedule a timeout after sending provider text")
...
assertContains(echo, "event: \"textOverTimeoutScheduled\"", "Tencent TextOver timeout scheduling should remain observable through redacted diagnostics")
assertContains(echo, "event: \"textOverTimeoutRecovered\"", "Tencent TextOver timeout recovery should remain observable through redacted diagnostics")
...
assertContains(echo, "prepareAudioSessionForTencentProviderPlayback(\n        preserveRecordingCategory:", "Echo should prepare an AVAudioSession route before Tencent provider speech")
...
assertContains(echo, "event: \"audioSessionCoordinatorPrepared\"", "Echo true-device diagnostics should show the provider playback audio-owner handoff")
...
assertContains(echo, "setDialogEngineLocalTTSPlaybackEnabled(false)", "Echo should disable Volcengine local TTS playback when Tencent owns audible speech through the binding-safe helper")
assertContains(echo, "setDialogEngineLocalTTSPlaybackEnabled(true)", "Echo should restore Volcengine local TTS playback outside the Tencent digital-human route through the binding-safe helper")
...
assertContains(featureFlags, "DJRunDigitalHumanLivePanelSmoke", "QA scenario registry should retain the digital-human smoke launch argument")
assertContains(featureFlags, "DJRunTencentDigitalHumanTextDriveSmoke", "QA feature registry should retain the true-device Tencent text-drive smoke launch argument")
assertContains(featureFlags, "DJRunTencentDigitalHumanPCMDriveSmoke", "QA feature registry should retain the true-device Tencent PCM audio-drive smoke launch argument")
assertContains(featureFlags, "DJRunTencentDigitalHumanBackendPCMDriveSmoke", "QA feature registry should retain the true-device backend Tencent PCM audio-drive smoke launch argument")
assertContains(appDelegate, "configuration.shouldEnableDigitalHumanLivePanel", "AppDelegate should use the centralized digital-human launch capability")
assertContains(appDelegate, "case .digitalHumanLivePanelSmoke", "AppDelegate should dispatch the digital-human smoke through the scenario registry")
