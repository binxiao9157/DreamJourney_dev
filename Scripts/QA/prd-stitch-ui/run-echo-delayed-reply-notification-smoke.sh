...
grep -Eq '"expiredDelayedReplyArrived"[[:space:]]*:[[:space:]]*true' "$RESULT_FILE" || fail "Expired delayed reply should transition to replied."
grep -Eq '"expiredDelayedReplyCleared"[[:space:]]*:[[:space:]]*true' "$RESULT_FILE" || fail "Expired delayed reply should clear local storage."
