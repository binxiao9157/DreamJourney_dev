...
assertContains(detail, "archive-audio-media-placeholder", "audio detail should show an empty state when the local file is missing")
assertContains(detail, "archive-audio-play-button", "audio detail should expose a play affordance when the local file exists")
assertContains(detail, "let hasPlayableAudio = item.hasResolvedLocalFile", "audio detail should derive playable state from resolved local file availability")
assertContains(detail, "playButton.isEnabled = hasPlayableAudio", "audio detail should avoid broken playback without a local file")
assertContains(detail, "guard let localPath = item.resolvedLocalFilePath else { return }", "audio playback should guard the resolved local path")
