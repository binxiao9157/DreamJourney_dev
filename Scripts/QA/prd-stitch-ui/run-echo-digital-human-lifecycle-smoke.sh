...
grep -Eq '"backgroundLeaseScheduled"[[:space:]]*:[[:space:]]*true' "$RESULT_FILE" || fail "Background release lease should be scheduled."
grep -Eq '"backgroundLeaseCancelled"[[:space:]]*:[[:space:]]*true' "$RESULT_FILE" || fail "Foreground restore should cancel the background lease."
grep -Eq '"backgroundLeaseExpired"[[:space:]]*:[[:space:]]*true' "$RESULT_FILE" || fail "A current background lease should expire in the UIQA path."
grep -Eq '"runtimeReleasedAfterGrace"[[:space:]]*:[[:space:]]*true' "$RESULT_FILE" || fail "Expired background lease should release the runtime."
