...
grep -Eq '"summaryDetail"[[:space:]]*:[[:space:]]*"确认整理后，会开始整理本次内容。"' "$RESULT_FILE" || fail "Product review-pending detail drifted."
...
grep -Eq '"reviewBatchAcknowledgementRendered"[[:space:]]*:[[:space:]]*true' "$RESULT_FILE" || fail "Acknowledged review batch should render the organizing state."
