...
grep -Eq '"timeLetterDraftDeliveryContractPersisted"[[:space:]]*:[[:space:]]*true' "$RESULT_FILE" || fail "Time-letter draft should persist the draft delivery contract."
grep -Eq '"timeLetterSealedDeliveryContractPersisted"[[:space:]]*:[[:space:]]*true' "$RESULT_FILE" || fail "Time-letter sealed state should persist the scheduled reminder contract."
grep -Eq '"timeLetterBackendPayloadScheduled"[[:space:]]*:[[:space:]]*true' "$RESULT_FILE" || fail "Time-letter backend payload should include scheduled local and in-app reminder fields."
...
grep -Eq '"videoTimelineThumbnailVisible"[[:space:]]*:[[:space:]]*true' "$RESULT_FILE" || fail "Video timeline thumbnail should be visible."
grep -Eq '"videoTimelinePlaceholderVisible"[[:space:]]*:[[:space:]]*true' "$RESULT_FILE" || fail "Video timeline placeholder should be visible."
...
grep -Eq '"timeLetterReminderPolicyVisible"[[:space:]]*:[[:space:]]*true' "$RESULT_FILE" || fail "Time-letter reminder policy should be visible."
...
grep -Eq '"releaseHiddenEntryPointsBlocked"[[:space:]]*:[[:space:]]*true' "$RESULT_FILE" || fail "Release mode should keep hidden audio/video entry points blocked while exposing time letters."
