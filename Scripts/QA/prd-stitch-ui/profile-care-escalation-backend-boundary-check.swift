...
let script = read("Scripts/QA/prd-stitch-ui/run-profile-care-escalation-boundary-smoke.sh")
let contractCheck = read("Scripts/QA/prd-stitch-ui/profile-care-escalation-contract-check.swift")
...
assertContains(featureFlags, "DJRunProfileCareEscalationBoundarySmoke", "QA scenario registry should expose care escalation boundary smoke launch argument")
assertContains(featureFlags, "enum QAProfileScenarioRunner", "Profile UIQA route and result persistence should use the shared runner")
assertContains(featureFlags, "static func selectRootProfileTab() -> Bool", "Profile UIQA runner should own the root tab route")
assertContains(featureFlags, "static func writeResult(", "Profile UIQA runner should own JSON result persistence")
assertContains(appDelegate, "case .profileCareEscalationBoundarySmoke", "AppDelegate should dispatch care escalation boundary smoke through the scenario registry")
...
assertContains(appDelegate, "QAProfileScenarioRunner.selectRootProfileTab()", "AppDelegate compatibility route should delegate to the shared Profile runner")
assertContains(appDelegate, "QAProfileScenarioRunner.writeResult(", "AppDelegate compatibility writer should delegate to the shared Profile runner")
...
assertContains(script, "run-installable-simulator-uiqa.sh", "script should use the guarded local simulator installer")
assertContains(script, "source \"$INSTALL_DIR/install.env\"", "script should consume the guarded installer environment")
