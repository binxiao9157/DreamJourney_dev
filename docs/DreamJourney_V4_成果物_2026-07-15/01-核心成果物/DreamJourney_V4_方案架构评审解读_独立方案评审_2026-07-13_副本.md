# DreamJourney V4 方案架构评审解读

版本：V1.4  
初版日期：2026-07-13  
更新日期：2026-07-14  
评审对象：`DreamJourney_V4_成果物_2026-07-12`  
...
| 成果物 | SHA-256 |
| --- | --- |
| Product Spec V4.0 | `30b61cc8d28d95e3d7a610944fee167cc6d76bc5f57c48af1e75ff590b194cfe` |
| V4 可执行开发路线图 | `f599ce8565673bb8dee00534baa75cbca9be18229a66e234183513849185af81` |
| 产品决策登记册 | `faf7caefe60f3654a2662503c300eb75e6a5352d2117e7cd8597d9e6ff8518be` |
| 当前实现证据矩阵 | `7bdd290f6dd155ac9d359abaeb01b9925919397939d2a2a226beb5fd32f967db` |

`路线执行注册表`、`路线追踪矩阵`、README 和 Round 5D 静态验收报告尚未按本基线重新生成或更新，因此只作为历史治理证据，不作为 V1.4 内容一致性和最终验收结论的权威来源。

## 1. 评审结论

### 1.1 一句话结论

DreamJourney V4 的核心架构方向是成立的：它把产品从“数字人功能集合”收敛为“由在世用户掌控证据、确认、纠正和分享边界的私人记忆与纪念人格系统”，并围绕单一数据权威、最小授权、可追溯异步任务和渐进迁移建立了较完整的目标架构。2026-07-14 产品评审已确认：经核验的成年近亲属可以为逝者建立 MemorialVault、担任日常主控人并邀请家庭成员共同补充材料；逝者本身不是登录账号。最新 Product Spec 18.2A 又把 Projection/Retrieval 后端路径的负载模型、时延预算、检索质量、数据正确性、可观测性、恢复和告警固化为可验收的 DFX 合同。
...
3. 多处状态机和字段定义存在前后不一致，尚不能直接作为数据库与 API 的唯一实现合同。
4. 41 项产品决定中已有 3 项确认：DR-004、DR-029 和 DR-030；其余大多仍是推荐或外部依赖，不能据此执行无关的不可逆设计和真实数据迁移。
5. Product Spec 18.2A 已给出 RTO <= 30 分钟、RPO <= 5 分钟等初始 DFX 目标，但这些仍是 `RECOMMENDED`，尚无生产等价压测、备份恢复或容量报告，不能被解释为已经达标或最终批准。
6. Projection SearchDocument、QueryPlan DTO、每类 Projector 映射、混合检索排序公式和金标检索语料仍未冻结为详细设计；Registry/Trace/README/Round 5D 报告也尚未同步最新路线图。

### 1.2 分层评审意见

| 评审层 | 结论 | 说明 |
| --- | --- | --- |
| 产品价值链 | 通过 | Owner Truth Loop 是本人和逝者纪念场景的共同底座；数字纪念方向已确认，Voice/DH 仍是独立高风险表达层 |
...
| 数据迁移与回滚 | 条件通过 | 方法完整且安全，但执行复杂度极高，需要真实规模、人员和演练数据 |
| Projection/Retrieval DFX | 条件通过 | 测量边界、百级负载、时延、质量、隔离、恢复和告警合同完整；尚无实测证据与索引详细设计 |
...
> 接受 V4 作为推荐目标架构和后续设计基线；接受 Product Spec 18.2A 作为 Projection/Retrieval 的初始测量和压测合同，但不把推荐阈值视为实测达标。允许按已确认合同设计本人和逝者 MemorialVault 的可逆 Schema、合成数据流程与私人文字原型。在统一状态合同、强身份/关系核验、首发地域与处理商、数据保留、索引详细设计和恢复指标获得批准前，不批准不可逆 Schema Contract 或真实敏感数据迁移；在逝者生前意愿、专项法律意见、供应商允许、AI 标识与近亲属争议机制关闭前，不批准真实逝者 Voice/Digital Human 训练和公开使用。
...
该设计应保留，并继续维持 Stage 3 可选模块，不应提前塞进 Owner 核心表。
...
2026-07-14 产品负责人已确认：Owner Truth Loop 是本人和逝者纪念场景的共同底座；逝者 Memorial Persona 与家庭共建是正式产品方向，Voice/DH 是可选、高风险且默认关闭的表达层。该决定已经写入 Product Spec 1.1、12.7、17.4、24.4B 与 DR-004，不再留给 UI 或研发临时选择。
...
成果物记录 41 个 Decision，其中 DR-004、DR-029、DR-030 共 3 项为 `CONFIRMED`；30 项为 `RECOMMENDED_PENDING`，5 项为 `EXTERNAL_REQUIRED`，3 项被拒绝。

这意味着当前可以批准的是“推荐方向”，不是最终产品合同。至少应优先关闭：

- 强身份和恢复；
- 成人、第三方、逝者与声音主体政策；
- 首发地域、处理商和禁训练/留存/删除条款；
- 账号删除恢复窗和备份披露；
- Publication/Visitor 是否进入近期范围；
- 家属私有查询授权模型；
- Voice/DH 成本、配额、退出和真机门；
- RPO/RTO、最小客户端版本、Migration 批次与批准角色。
...
Product Spec 18.2A 已补充百级负载、RTO <= 30 分钟、RPO <= 5 分钟、Projection 重建和24小时稳定性等初始目标，关闭了“完全没有参数”的文档缺口。但这些目标尚未绑定到真实部署和恢复机制，仍未冻结：
...
13 个 Package、115 个 Work Item、六条迁移轨道和五类 Gate 已接近一个正式平台项目。若团队较小，最大的风险不是架构不够完整，而是所有控制都只存在于文档。

建议每个 Release Increment 只激活少量 Work Item，并强制指定：
...
6. **仍待产品/隐私关闭**：被授权者查询正文的主控人可见性、会话 TTL、删除、举报保留和审计最小字段，需要随 DR-038/036 形成可执行参数。
...
20. 谁负责关闭其余未确认或依赖外部门的 Decision，各自截止 Gate 是什么？
21. 真实数据首次进入系统前，必须完成哪些 G2/G3/G4 证据？
22. 最低客户端版本和强制升级策略是什么？
23. 第一次真实迁移只跑哪个 Vault/cohort，最大恢复时间是多少？
24. Optional lane 失败时，文字核心的用户体验和客服解释是什么？
...
### 18.7 文档基线一致性 Finding

当前 Product Spec 与路线图已经包含 DFX 修订，但生成和交付视图尚未同步：

- Registry 内嵌路线图 SHA 为 `a1ad8295...`，本次权威路线图 SHA 为 `f599ce85...`；
- README 与 Round 5D 报告记录的 Trace/Registry SHA 已不等于目录内当前文件；
- 评审与验收清单尚未纳入 `T_retrieval`、Projection lag、检索质量和百级负载门。

这不推翻 Product Spec 18.2A 的架构内容，但会阻断“完整成果物已同步、静态验收已通过”的结论。应重新生成 Registry/Trace、更新验收清单、重跑全部静态检查，再发布新的静态验收补充报告；历史 Round 报告保持不变。

### 18.8 评审结论

> Product Spec 18.2A 已把 Projection/Retrieval 从抽象模块补成可测量的后端 DFX 合同，文档架构缺口可以关闭。当前只能批准负载模型、SLI/SLO、正确性门、Telemetry 和验收方法作为研发基线；由于检索详细设计、生产等价压测、金标质量、灾备演练和生成成果物同步尚未完成，性能与容量实现状态继续保持 `PLANNED/STOP`，不得标记 `PROD_VERIFIED`。
...
- 重新生成 Registry/Trace，更新验收清单、README 和静态验收报告，使哈希与本版权威成果物一致。
...
其最强之处是边界意识、失败语义、本次补齐后的本人/逝者主体分离，以及已经可测量的 Projection/Retrieval DFX 合同；最弱之处是状态合同尚未完全统一、检索详细设计和实测证据仍缺失、逝者 Voice/DH 无生前授权时缺少可批准法律依据，以及方案规模远超普通 MVP 的实施能力。

因此本次建议为：

> **目标架构条件性通过；产品定位、人物记忆本体、纪念人格权利模型和 Projection/Retrieval DFX 测量合同已在架构层关闭。允许纪念档案可逆 Schema、检索详细设计和合成数据压测；状态合同、身份/关系核验、数据权利门、生产等价 DFX 证据与灾备演练关闭后，才能进入真实私人资料试点。无生前明确授权的逝者 Voice/Digital Human 继续保持 No-Go，Publication 和不可逆迁移仍需各自外部门。**

## 附录：评审依据

本版内容判断使用以下四份权威成果物，并以文首 SHA-256 锁定基线：

- [Product Spec V4](./DreamJourney_V4_产品定义与目标架构_Product_Spec_V4.0.md)
- [产品决策登记册](./DreamJourney_V4_产品决策登记册_V1.0.md)
- [V4 可执行开发路线图](./2026-07-12-dreamjourney-v4-executable-development-roadmap.md)
- [当前实现证据矩阵](./DreamJourney_V4_当前实现证据矩阵_V1.0.md)

以下文件是历史治理或待同步视图，不用于证明 V1.4 最新基线已经完成静态验收：

- [评审与验收清单](./DreamJourney_V4_评审与验收清单_V1.0.md)：尚未纳入 18.2A DFX；
- [路线执行注册表](../02-追踪与执行/DreamJourney_V4_路线执行注册表_V1.0.json)：内嵌路线图 SHA 尚未刷新；
- [路线追踪矩阵](../02-追踪与执行/DreamJourney_V4_路线追踪矩阵_V1.0.md)：应随 Registry 重新生成；
- [Round 5D 最终静态验收报告](../05-交付记录/DreamJourney_V4_Round5D_最终静态验收报告.md)：保留为历史报告，不能覆盖本次修订后的重新验收。
...
全部确认后，应将最终选择同步回 Product Spec、产品决策登记册、路线图、验收清单和发布策略；只填写本章而未同步权威产品合同，不视为研发范围已经正式变更。
