...
| FR-OPS-001 | P0 | `IMPLEMENTED` runtime/readiness states | `IMPLEMENTED` typed jobs/outbox/dead-letter/replay/启动预检 | CONFIRMED: DR-024; OPEN: DR-035 | COS 后的 Worker 启用、故障演练 | HIDDEN_QA | `IMPLEMENTED` | Cross-cutting Operations | live schema readiness 与四类 Owner Truth Worker fail-closed 预检已部署；线上 smoke 证明 B1 未完成时全部拒绝启动，生产 Worker/scheduler 仍 disabled，不能宣称异步效果自动完成 |
