...
- iOS 使用独立的后端精提取水位，避免本地轻量提取推进总 sessionCount 后长期不再调用后端。
- 只有 personal/self persona 可以使用本地 KBLite fallback；family persona 禁止把 viewer 个人知识作为降级上下文。
- Context Packet identity 与当前 user/persona/digital-human gate 不一致时不得提交生成。
...
- iOS structured turns、精提取节流水位、本地 confidence filter 和 persona-bound fallback policy。
...
- [x] 实现并测试后端证据完整性和 Context P0 policy。
- [x] 实现并测试 iOS structured turns、提取节流和 persona-bound 本地生成门禁。
- [x] 接入跨仓库 QA/release gate，运行非真机构建和回归。
- [x] 更新状态文档、关闭 ledger、分别提交两仓库。
...
- family persona 请求失败不会使用当前 viewer 的个人 KBLite；错 user/persona/digital-human packet 会被拒绝。
