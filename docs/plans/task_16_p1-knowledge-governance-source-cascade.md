...
- [x] 固定治理 action schema、合法状态转换、纠正字段 allowlist 和来源级联语义。
- [x] 实现后端 governance builder、endpoint、owner 路由分类和 memory/Postgres 兼容。
- [x] 覆盖 confirm/reject/correct/deleteSource、幂等、revision conflict、非法 persona/target/source 负向测试。
- [x] iOS 增加治理 action/response/metadata 模型和 backend client。
- [x] iOS 同步协调器串行提交治理动作、应用权威 snapshot，并防止用户/角色切换后的旧回调写入。
- [x] Echo/Context 验证 rejected/superseded 继续被过滤，纠正实体可进入目标 persona Context。
- [x] 增加跨仓库 smoke、release regression 可选 gate、设计/状态文档和 Ledger 证据。
- [x] 运行后端全量验证、iOS 模型检查、Simulator/iPhoneOS generic build、`git diff --check`，分别提交两仓库。
...
- 后端：`scripts/run-backend-knowledge-governance-source-cascade-smoke.sh`，217 条 deterministic tests 通过；`scripts/verify_backend.sh`，243 条 tests 通过。
- iOS：governance model/client/outbox/coordinator/boundary、three-way/proposal/context smoke 全部通过。
- 组合 gate：`RUN_KNOWLEDGE_GOVERNANCE_GATE=1`。
- Release regression：`tmp/visual-qa/prd-stitch-ui/release-regression/20260711-task16-knowledge-governance-v4/report.md`。
- 构建：Simulator Debug workspace 与 generic iPhoneOS 无签名构建通过。

## 剩余边界

- 当前没有公开知识管理/确认/纠正 UI；后续必须先由 PRD 和 Stitch 确认入口与交互。
- 新档案统一使用 `memoryArchiveItem + archiveItem.id`。历史 `archiveImageAnalysis + session-*` 不会自动匹配，需独立迁移任务。
- 未做真实 Postgres deployed smoke、线上部署、真机和 provider 质量验证。
- operation payload hash、change feed compaction 和物理审计清理仍不在本任务范围。
