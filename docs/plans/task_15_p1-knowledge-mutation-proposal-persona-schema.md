...
- [x] 固定 proposal/persona 合同并建立回归样例。
- [x] 实现后端 proposal builder、稳定 ID、去重、关系和 metadata。
- [x] 实现 Context persona fact policy 与负向回归。
- [x] 实现 iOS proposal 解析、identity-bound 合并和旧合同兼容。
- [x] 接入跨仓库 QA gate，运行非真机构建和回归。
- [x] 更新状态文档、关闭 ledger、分别提交两仓库。

## 验证证据

- 后端 verify：213 项 unittest、py_compile、FastAPI、knowledge delta/V2/evidence smoke 通过。
- Release regression：`tmp/visual-qa/prd-stitch-ui/release-regression/20260711-knowledge-proposal-persona-final3/report.md`。
- Generic iPhoneOS：`tmp/visual-qa/prd-stitch-ui/iphoneos-generic-build/20260711-knowledge-proposal-persona-final/report.md`。
- 模拟器通知 smoke 随 release regression 通过；未运行真机。

## 递归 Ledger

- `L20260711-030653-15`
- 7/7 problems、7/7 tickets、7/7 checks closed。
