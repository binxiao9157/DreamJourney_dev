...
- [x] 后端 proposal 为不同 source turn 生成不同 canonical refs。
- [x] 非用户 turn、越界 turn 仍被 evidence policy 拒绝。
- [x] iOS quick extraction 和后端 request 不再新增 `conversationSession`。
- [x] 对话照片不再新增 `archiveImageAnalysis`，且 source ID 不含绝对路径。
- [x] Source audit 只返回计数/策略，不返回实体正文或 source ID。
- [x] Archive delete 仍只匹配 `memoryArchiveItem`，legacy 对话照片不会误级联。
- [x] 后端全量、知识组合 gate、Simulator/generic iPhoneOS build 和 release regression 通过。
- [x] 分仓提交推送；后端已部署并通过线上 Postgres audit smoke；不做真机。
