...
- [x] 后端 fingerprint helper、错误类型与 operation receipt。
- [x] Memory/Postgres 新记录、legacy 兼容、事务回滚和 purge。
- [x] 相同 payload 重放、不同 payload/schema/kind 冲突、仅 baseRevision 改变仍幂等。
- [x] Governance 原 action 重放与不同 action 冲突。
- [x] Archive 无级联删除原样重放与不同 item 冲突。
- [x] iOS 结构化错误消费和普通 mutation 恢复。
- [x] Governance rotate/quarantine，且后续队列可继续。
- [x] Outbox Codable 向后兼容和 operation ID 规范化。
- [x] QA gate、release regression、知识架构与状态文档更新。
- [x] 后端全量测试、`git diff --check`、iOS Simulator/generic iPhoneOS 构建通过。
- [x] 后端和 iOS 分仓提交，不推送、不部署。
...
- Closure ledger：`L20260711-100251`，4 problems / 4 tickets / 4 success checks。
- 后端：250 tests、compile、FastAPI 与 knowledge smokes 通过。
- 跨仓治理 gate：224 tests 与 iOS model/client/outbox/coordinator checks 通过。
- Release regression：`tmp/visual-qa/prd-stitch-ui/release-regression/20260711-task17-knowledge-operation-receipt/report.md`。
- Simulator workspace build 与 generic iPhoneOS arm64 build 通过。
