...
- [x] 默认/legacy event 不进入 Widget；只有显式 summaryAllowed + confirmed personal owner event 可进入。
- [x] 编码快照不含 raw owner/event/source ID、description 或 sourceRefs。
- [x] owner digest 不匹配、登出、旧 schema、损坏文件都返回空 entry。
- [x] 旧 generation publish 在切换/登出后被拒绝。
