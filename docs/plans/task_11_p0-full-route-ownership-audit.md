...
- [x] 生成完整业务路由清单并先补“所有路由必须分类”的失败测试。
- [x] 实现 route ownership registry 和 owner/system principal-bound 决策。
- [x] 补高风险读写路由的错误 principal 403 集成测试。
- [x] 移除 iOS 全局 `dispatch-due` 调用，保留 mailbox 拉取。
- [x] 增加审计报告、HTTP smoke、release regression 可选 gate 和状态文档。
- [x] 完成后端/iOS 全量验证、提交推送、重新部署和线上 Postgres smoke。
...
- Ledger ID：`L20260710-222422`。
- 状态：进行中。

## 本地验证证据

- 后端：177 tests passed，FastAPI smoke passed。
- HTTP：auth session、cross-account delegated、route ownership audit 三条本地 smoke passed。
- iOS：时间信件 reminder simulator smoke passed。
- Release QA：全部静态 guard 与 iPhoneOS generic Debug build passed。
- 报告：`tmp/visual-qa/prd-stitch-ui/release-regression/20260710-task11-ownership-audit/report.md`。

## 线上 Postgres 证据

- 后端提交：`275a4c2 feat: enforce route ownership principals`，已推送 `origin/main` 并部署。
- `/health`：`200`，`environment=production`，`store=postgres`。
- 容器：仅 API 重建；Postgres 保持 healthy，Redis 保持 running。
- 时间信件 timer：`enabled`、`active`。
- Auth smoke：access/refresh 轮换、防重放、注销和 principal-bound mismatch 403 通过。
- Cross-account smoke：家庭邀请、关怀、时间信件 delegated 通过，伪造 viewer 403。
- Route audit smoke：`routeCount=54`、`unclassifiedCount=0`、owner path/body 403、system-only user 403、`ownershipMode=shadow`。
- 脱敏报告目录：`tmp/visual-qa/prd-stitch-ui/deployed-route-ownership/20260710-275a4c2/`。
