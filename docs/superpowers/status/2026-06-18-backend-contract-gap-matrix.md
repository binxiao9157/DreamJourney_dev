...
| Echo delayed reply dispatch | `/echo/delayed-replies/dispatch-due` | accepted on selected release-like backend by run `20260618-deployed-echo-dispatch-contract-accepted-211732`; earlier runs `20260618-deployed-echo-dispatch-contract-210536` and `20260618-deployed-echo-dispatch-contract-rerun-report-211438` were blocked with HTTP 405 before redeploy | service-side due sweep moves scheduled replies to `readyForProvider` / `queued` and records `dispatchAttemptedAt` without claiming APNs provider delivery; APNs delivery and true-device notification arrival remain external gates | `echo-delayed-reply-dispatch-contract-check.swift`, backend `EchoDelayedReplyAPITests`, backend `PostgresStoreTests`, `backend-postgres-persistence-check.py`, accepted deployed run `20260618-deployed-echo-dispatch-contract-accepted-211732` |
...
- Keep APNs provider delivery and true-device notification arrival as external gates after selected-backend `POST /echo/delayed-replies/dispatch-due` acceptance; redeployed run `20260618-deployed-echo-dispatch-contract-accepted-211732` is accepted.
