...
| Password change | `/auth/password` | backend route present locally; selected-environment deployment and auth acceptance needed | backend now hashes password credentials and requires old-password verification in local code; iOS login password participation is implemented through `/auth/login` when backend auth is configured; local release-like runner now checks `passwordChangeStatus`, `passwordOldLoginStatus`, and `passwordNewLoginConfigured`; iOS password-change page and client contract remain hidden and not public release until deployed route, selected-environment login/password acceptance, security review, and true-device acceptance are complete | `login-password-contract-check.swift`, `profile-password-change-check.swift`, `release-like-backend-acceptance-check.swift`, backend `PasswordAPITests` |
...
- Deploy `/auth/password`, rerun release-like backend acceptance for the new password fields, and complete selected-environment login/password acceptance plus security review before promoting password change out of hidden release gates.
