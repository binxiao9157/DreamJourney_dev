...
- 修改密码已有隐藏 iOS 壳层和本地后端 `/auth/password` 哈希凭据合同；仍需登录密码参与、线上部署、安全审查和真机验收后才能公开。
...
| 账号中心范围 | 已明确头像、名称、性别、地区、手机号和 App 内修改密码；本地后端 `/auth/password` 合同已补，仍需登录密码参与、线上部署、安全审查和真机验收 | 产品 + 后端 |
