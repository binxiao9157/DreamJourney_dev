...
| 回响语音 | 以 `开始语音` 为核心的语音交互；第 3 次用户最终语音后进入等待回信默认策略 | 真机麦克风验收、生产语音 SDK 质量、等待回信策略的产品确认。 |
...
- 已在 `EchoViewModel` 中定义稳定的状态流转。
- 已覆盖空闲、聆听中、思考中、等待回信、已回信、失败等状态。
- 已增加等待回信策略回归检查：第 3 次用户最终语音后进入等待回信。
- 仍需真机麦克风路径和生产语音 SDK 行为验收。

需要 PRD / 产品决策：

- 是否接受当前默认策略：第 3 次用户最终语音后进入等待回信。
- “2-3 轮”长期产品定义到底指用户发言轮、AI 回复轮，还是一次完整对话交换。
- 后续是否从固定第 3 轮升级为根据情绪/内容自适应。
- 等待时长和界面文案。
