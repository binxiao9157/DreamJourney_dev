...
Production voice SDK note: 生产语音 SDK key now resolves through build settings (`VOLCENGINE_APP_ID`, `VOLCENGINE_APP_KEY`, `VOLCENGINE_APP_TOKEN`) instead of hard-coded plist placeholders. `DialogEngineManager` fails early when production credentials are missing or still placeholders, and `run-true-device-voice-preflight.sh` is the entrypoint before true-device voice acceptance. The current iPhoneOS no-sign compile passed on 2026-06-18, and the connected iPhone is visible to `xcodebuild -showdestinations`; signed install/run is still blocked by an invalid Apple Development certificate. This is configuration and compile readiness only; production voice quality is still not accepted until a real device run captures microphone, ASR, TTS, foreground/background, and SDK error evidence.
...
- 真机验收：not accepted; device detection and iPhoneOS no-sign compile passed, signed install/run is blocked by invalid development certificate
- 语音 SDK 生产质量验收：not accepted; production config injection and preflight gate are implemented, real VolcEngine credentials and true-device evidence remain required
- App Store / TestFlight 签名链路验收：not accepted; current local Apple Development certificate is invalid for code signing
