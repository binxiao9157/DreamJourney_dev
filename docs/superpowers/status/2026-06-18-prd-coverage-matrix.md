...
Echo waiting reply note: PRD updated to ten-round/adaptive policy; the old third-turn default policy has been superseded. PRD now says one user speech plus one AI reply counts as one round; the default waits after 10 rounds; emotion/content signals can trigger earlier; delay is 5-10 minutes. In-app state, local notification, device-token registration, and delayed-reply backend persistence are implemented through `EchoDelayedReplyStore`, `EchoDelayedReplyNotificationScheduler`, `PushDeviceTokenStore`, `DreamJourneyBackendClient.registerPushDeviceToken`, and `DreamJourneyBackendClient.scheduleEchoDelayedReplyPush`. Deployed backend acceptance run `20260618-deployed-push-device-token-contract-rerun-205018` verified `echoDelayedReplyDeviceTokenId` and `echoDelayedReplyPushProviderState=pending`. APNs provider delivery, service-side scheduled dispatch, and true-device notification acceptance remain open.
...
Backend contract note: iOS/backend parity is tracked in `docs/superpowers/status/2026-06-18-backend-contract-gap-matrix.md`. `/profile`, `/auth/login`, `/auth/password`, archive persona visibility fields, care snapshot state fixtures, `/devices/push-token`, and delayed reply `deviceTokenId` persistence are accepted on the selected release-like backend with runs `20260618-selected-backend-latest-contracts-after-deploy-r2` and `20260618-deployed-push-device-token-contract-rerun-205018`. `/auth/password` still needs security review before public exposure. `/echo/delayed-replies` still needs APNs provider delivery, scheduled dispatch worker acceptance, and true-device notification acceptance before remote push can be called complete.
...
- 后续如有后端合同变化，需要 rerun `run-release-like-backend-acceptance.sh`; deployed push-token registration and delayed-reply `deviceTokenId` persistence are accepted, while APNs provider delivery, scheduled dispatch worker, and true-device notification arrival are still external gates.
