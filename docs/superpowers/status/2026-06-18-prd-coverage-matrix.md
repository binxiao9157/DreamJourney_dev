...
Echo waiting reply note: PRD updated to ten-round/adaptive policy; the old third-turn default policy has been superseded. PRD now says one user speech plus one AI reply counts as one round; the default waits after 10 rounds; emotion/content signals can trigger earlier; delay is 5-10 minutes. In-app state, local notification, device-token registration contract, and backend-ready push contract are implemented through `EchoDelayedReplyStore`, `EchoDelayedReplyNotificationScheduler`, `PushDeviceTokenStore`, `DreamJourneyBackendClient.registerPushDeviceToken`, and `DreamJourneyBackendClient.scheduleEchoDelayedReplyPush`. Deployment route parity, APNs provider delivery, and true-device notification acceptance remain open.
...
Backend contract note: iOS/backend parity is tracked in `docs/superpowers/status/2026-06-18-backend-contract-gap-matrix.md`. `/profile`, `/auth/login`, `/auth/password`, archive persona visibility fields, and care snapshot state fixtures are accepted on the selected release-like backend with run `20260618-selected-backend-latest-contracts-after-deploy-r2`. Local backend code and release-like scripts now cover `/devices/push-token` plus delayed reply `deviceTokenId`, but selected deployed backend acceptance must be rerun after deployment; latest deployed push-token rerun `20260618-deployed-push-device-token-contract-203820` failed with HTTP 404 for `POST /devices/push-token`, so remote Echo push readiness remains blocked. `/auth/password` still needs security review before public exposure. `/echo/delayed-replies` also needs APNs provider delivery and true-device notification acceptance before remote push can be called complete.
...
- release-like FastAPI/Postgres and simulator remote-backend acceptance passed with `20260618-deployed-postgres-acceptance-after-deploy` and latest contract run `20260618-selected-backend-latest-contracts-after-deploy-r2`.
- 后续如有后端合同变化，需要 rerun `run-release-like-backend-acceptance.sh`; `/devices/push-token` is a new local contract and needs selected deployed backend rerun after deployment. Latest deployed rerun `20260618-deployed-push-device-token-contract-203820` failed with HTTP 404 for this route, so deploy the new backend route before claiming remote Echo push readiness.
