...
| 个人资料管理 | implemented for profile fields; login now uses the typed strong identity challenge/verify contract and fails closed while the real identity provider is unavailable; legacy password login and password change UI remain retired/hidden | yes for profile fields; login awaits a G4-verified identity provider | `LoginViewController`, `BackendIdentityChallenge`, `ProfileSettingsViewController`, `login-password-contract-check.swift`, `product-v4-identity-challenge-client-check.py`, backend `IdentityBindingAPITests` | real identity provider integration, auth/security review, true-device acceptance, explicit password-change product decision |
...
Profile settings note: name/gender/region validation, inline save states, local persistence, and the dedicated iOS `/profile` sync contract are covered by `profile-settings-save-state-check.swift` and `profile-account-fields-check.swift`. The selected backend accepts `POST /profile` and `GET /profile/{user_id}` in release-like run `20260618-selected-backend-latest-contracts-after-deploy-r2`. Strong identity challenge participation is covered by `login-password-contract-check.swift` and the Product V4 identity guards: the visible login form contains only the phone identity, requires the typed runtime capability, and uses `/v2/auth/challenges` plus verify. Legacy `/auth/login` is retired in production. Password change hidden shell, also tracked as Password change hidden shell for release guard evidence, and the historical iOS `/auth/password` client contract remain covered by `profile-password-change-check.swift`; backend `PasswordAPITests` are compatibility evidence only and do not reopen password login. Password change remains hidden; a real identity provider, auth/security review, and true-device acceptance remain open, plus an explicit public-release decision.

Backend contract note: iOS/backend parity is tracked in `docs/superpowers/status/2026-06-18-backend-contract-gap-matrix.md`. `/profile`, archive persona visibility fields, care snapshot state fixtures, `/devices/push-token`, delayed reply `deviceTokenId` persistence, and `/echo/delayed-replies/dispatch-due` are accepted on the selected release-like backend with runs `20260618-selected-backend-latest-contracts-after-deploy-r2`, `20260618-deployed-push-device-token-contract-rerun-205018`, and `20260618-deployed-echo-dispatch-contract-accepted-211732`. The old `/auth/login` and `/auth/password` results are retained as historical compatibility evidence only; current production login requires the typed strong identity challenge contract, while password change remains hidden. Earlier deployed runs `20260618-deployed-echo-dispatch-contract-210536` and `20260618-deployed-echo-dispatch-contract-rerun-report-211438` returned HTTP 405 for dispatch-due before the latest backend redeploy and are retained as recovered deployment-drift evidence. `/echo/delayed-replies` still needs APNs provider delivery and true-device notification acceptance before remote push can be called complete.
