...
- 后端 `f8cf209` 已将 ClamAV 纳入启动期本地依赖探测：即使误配置为 `clamav`，缺少可执行文件、可读签名库或探测失败时也会返回 `contentSafetyScannerUnavailable` 并拒绝开启采集。该探测不发送用户字节；生产容器内已验证其 fail-closed 行为。后续 `A1.1` 将其收敛为默认关闭的内部 ClamAV sidecar profile：显式配置 `OWNER_TRUTH_MEDIA_CLAMAV_HOST=clamav` 后，API/Worker 通过 clamd `INSTREAM` 探测和扫描；离线、超时、签名库异常或未知响应均在对象存储写入前隔离上传。对应实现/运行说明在后端 `docs/backend/2026-08-08-owner-truth-media-clamav-sidecar-a1-1.md`。
- 后端 `3adf6fb` 已部署，Compose profile 已通过服务器配置解析；当前仍不启动 ClamAV sidecar，原因是服务器可用内存约 `2.5 GiB`，不应在生产 API 旁承担签名库加载/重载风险。媒体 runtime 继续为 `disabled/runtimeDisabled`，没有误开启摄入。
- 仍未达到真实 Provider 完成态：服务器尚无 COS bucket、region、HTTPS endpoint、最小权限 SecretId/SecretKey、SSE/保留策略和 closed-pilot 测试租户配置；ClamAV sidecar 也尚未在容量充足的服务器上常驻启用并完成 smoke。未提供这些输入并完成扫描器运行条件前，runtime 保持 fail-closed，不能将 A1 标记为已上线。
...
- 后端已补齐明确不具资格的客户端草稿 hard deny 与 voice-profile 测试存储隔离；提交 `23ba7a7` 已于 2026-08-08 部署。线上 route runtime-only 审计确认 173 条路由全部分类、无未分类项且全局模式仍为 `shadow`；该无写入探针不替代缺少 V2 OTP 测试凭据时的跨账号完整审计。
