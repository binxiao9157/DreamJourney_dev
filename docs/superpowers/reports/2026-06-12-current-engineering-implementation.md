...
- 旧演示信件清理只认明确 `roadshow_` ID，不再按收件人、标题或正文内容删除真实信件。
...
- 当前登录用户可通过手机号匹配或本机 override 解析为家庭成员身份；只有 `active` + `accepted` 成员能成为关怀看板访问者，未显式传入成员时默认按当前访问者身份过滤。
- 邀请接受和成员权限撤回已有本地服务层：手机号匹配才允许接受邀请，已撤回邀请拒绝；撤回某成员权限时，全体亲友授权会转换成剩余成员白名单，单成员授权会移除该成员。
- Family 页面已接入邀请/权限 UI：搜索框/加号可接受手机号、邀请码或邀请链接；成员行可长按或左滑撤回访问；pending/revoked 成员会展示状态并阻止进入成员视角关怀看板。
- `FamilyMember` 承接后端 `accessStatus` / `invitationStatus`；亲友可见范围选择器只列出 active + accepted 成员，避免把待接受或已撤回成员纳入分享/看板授权。
- Postgres 和 in-memory 后端的邀请码接受路径均拒绝 revoked 成员，并对已 accepted 成员保持幂等返回。
- CareDashboard 输入按 `.careDashboard` surface 和目标成员可见性过滤。
- 看板展示观测窗口、数据覆盖摘要、观测天数、7 天脱敏趋势、脱敏观察报告和需关注信号。
- 当前风险等级、指标和趋势只使用以当前时间为终点的最近 7 天授权对话；过期历史不会污染当前关怀判断，近 7 天无数据时保持“数据不足”。
- 后端 CareSnapshot 写入要求完整聚合字段，确保 iOS 可严格解码；summary/suggestions/trend 等允许字段会拒绝 raw/transcript/原始对话标记、换行和过长文本。
