...
| Privacy Scope 模型 | 90% | `ConversationTurn`、`Stage1MailboxMemoryInput`、`DialogMessage`、MemoryArchive、TimeMailbox、KBLite v2 实体已携带 `privacyMetadata`；新增 `FamilyMemberVisibility`，旧数据默认 all-family 可见且 scope 保守迁移。 |
...
| CareDashboard/Family Sync 阶段2 | 55% | Family share package、FamilyRepository 已改用 familyCircle sanitized graph；CareDashboard transcript 入口按 `.careDashboard` 和目标成员可见性过滤，只输出脱敏统计输入。 |
...
3. 补 Family/CareDashboard 的授权 UI：为记忆/对话选择具体可见成员，并把 `viewerFamilyMemberID` 接到真实登录亲友身份。
