...
private extension AppDelegate {
    func configureUIQASmokeHarnessIfNeeded() {
        ...
        case .ownerTruthInterviewOrchestrationSmoke:
        ...
        case .ownerTruthKnowledgeDimensionConfirmationSmoke:
            scheduleUIQAScenario(scenario) { $0.runOwnerTruthKnowledgeDimensionConfirmationSmoke() }
        case .ownerTruthKnowledgeRecommendationPlanSmoke:
            scheduleUIQAScenario(scenario) { $0.runOwnerTruthKnowledgeRecommendationPlanSmoke() }
        case .ownerTruthInterviewBoundarySmoke:
            scheduleUIQAScenario(scenario) { $0.runOwnerTruthInterviewBoundarySmoke() }
    ...
    func runOwnerTruthInterviewOrchestrationSmoke(retryCount: Int = 0) {
        guard OwnerTruthCandidateReviewQAGate.isEnabled else {
            OwnerTruthInterviewOrchestrationUIQASmoke.writeFailure("qaGateDisabled")
            return
        }
        guard let userID = UserManager.shared.currentUser?.id,
              let accountLease = AccountLeaseRuntime.shared.capture(forSubjectId: userID),
              ...
              AccountLeaseRuntime.shared.validate(accountLease, at: .request).allowed else {
            guard retryCount < 20 else {
                OwnerTruthInterviewOrchestrationUIQASmoke.writeFailure("accountLeaseUnavailable")
                return
            }
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.2) { [weak self] in
                self?.runOwnerTruthInterviewOrchestrationSmoke(retryCount: retryCount + 1)
            }
            return
        }
        guard let keyWindow = UIApplication.shared.connectedScenes
            ...
            .first(where: { $0.isKeyWindow }),
              keyWindow.rootViewController is WarmTabBarController else {
            guard retryCount < 20 else {
                OwnerTruthInterviewOrchestrationUIQASmoke.writeFailure("mainRootUnavailable")
                return
            }
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.2) { [weak self] in
                self?.runOwnerTruthInterviewOrchestrationSmoke(retryCount: retryCount + 1)
        ...
        let controller = OwnerTruthInterviewOrchestrationUIQASmoke.makeViewController(
            accountLease: accountLease
        )
        keyWindow.rootViewController = UINavigationController(rootViewController: controller)
        keyWindow.makeKeyAndVisible()
        print("[UI_QA] OwnerTruthInterviewOrchestrationSmoke started")
    }

    func runOwnerTruthInterviewTopicSwitchSmoke(retryCount: Int = 0) {
        guard OwnerTruthCandidateReviewQAGate.isEnabled else {
            OwnerTruthInterviewTopicSwitchUIQASmoke.writeFailure("qaGateDisabled")
            return
        }
        guard let userID = UserManager.shared.currentUser?.id,
              let accountLease = AccountLeaseRuntime.shared.capture(forSubjectId: userID),
              ...
              AccountLeaseRuntime.shared.validate(accountLease, at: .request).allowed else {
            guard retryCount < 20 else {
                OwnerTruthInterviewTopicSwitchUIQASmoke.writeFailure("accountLeaseUnavailable")
                return
            }
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.2) { [weak self] in
                self?.runOwnerTruthInterviewTopicSwitchSmoke(retryCount: retryCount + 1)
            }
            return
        }
        guard let keyWindow = UIApplication.shared.connectedScenes
            ...
            .first(where: { $0.isKeyWindow }),
              keyWindow.rootViewController is WarmTabBarController else {
            guard retryCount < 20 else {
                OwnerTruthInterviewTopicSwitchUIQASmoke.writeFailure("mainRootUnavailable")
                return
            }
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.2) { [weak self] in
                self?.runOwnerTruthInterviewTopicSwitchSmoke(retryCount: retryCount + 1)
        ...
        keyWindow.makeKeyAndVisible()
        OwnerTruthInterviewTopicSwitchUIQASmoke.start(
            accountLease: accountLease,
    ...
    func runOwnerTruthKnowledgeRecommendationPlanSmoke(retryCount: Int = 0) {
        guard OwnerTruthCandidateReviewQAGate.isEnabled else {
            OwnerTruthKnowledgeRecommendationPlanUIQASmoke.writeFailure("qaGateDisabled")
            return
        }
        guard let userID = UserManager.shared.currentUser?.id,
              let accountLease = AccountLeaseRuntime.shared.capture(forSubjectId: userID),
              ...
              AccountLeaseRuntime.shared.validate(accountLease, at: .request).allowed else {
            guard retryCount < 20 else {
                OwnerTruthKnowledgeRecommendationPlanUIQASmoke.writeFailure("accountLeaseUnavailable")
                return
            }
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.2) { [weak self] in
                self?.runOwnerTruthKnowledgeRecommendationPlanSmoke(retryCount: retryCount + 1)
            }
            return
        }
        guard let keyWindow = UIApplication.shared.connectedScenes
            ...
            .first(where: { $0.isKeyWindow }),
              keyWindow.rootViewController is WarmTabBarController else {
            guard retryCount < 20 else {
                OwnerTruthKnowledgeRecommendationPlanUIQASmoke.writeFailure("mainRootUnavailable")
                return
            }
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.2) { [weak self] in
                self?.runOwnerTruthKnowledgeRecommendationPlanSmoke(retryCount: retryCount + 1)
        ...
        print("[UI_QA] OwnerTruthKnowledgeRecommendationPlanSmoke started")
    }

    func runOwnerTruthKnowledgeDimensionConfirmationSmoke(retryCount: Int = 0) {
        guard OwnerTruthCandidateReviewQAGate.isEnabled else {
            OwnerTruthKnowledgeDimensionConfirmationUIQASmoke.writeFailure("qaGateDisabled")
            return
        }
        guard let userID = UserManager.shared.currentUser?.id,
              let accountLease = AccountLeaseRuntime.shared.capture(forSubjectId: userID),
              ...
              AccountLeaseRuntime.shared.validate(accountLease, at: .request).allowed else {
            guard retryCount < 20 else {
                OwnerTruthKnowledgeDimensionConfirmationUIQASmoke.writeFailure("accountLeaseUnavailable")
                return
            }
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.2) { [weak self] in
                self?.runOwnerTruthKnowledgeDimensionConfirmationSmoke(retryCount: retryCount + 1)
            }
            return
        }
        guard let keyWindow = UIApplication.shared.connectedScenes
            ...
            .first(where: { $0.isKeyWindow }),
              keyWindow.rootViewController is WarmTabBarController else {
            guard retryCount < 20 else {
                OwnerTruthKnowledgeDimensionConfirmationUIQASmoke.writeFailure("mainRootUnavailable")
                return
            }
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.2) { [weak self] in
                self?.runOwnerTruthKnowledgeDimensionConfirmationSmoke(retryCount: retryCount + 1)
        ...
        print("[UI_QA] OwnerTruthKnowledgeDimensionConfirmationSmoke started")
    }

    func runOwnerTruthInterviewBoundarySmoke(retryCount: Int = 0) {
        guard OwnerTruthCandidateReviewQAGate.isEnabled else {
            OwnerTruthInterviewBoundaryUIQASmoke.writeFailure("qaGateDisabled")
            return
        }
        guard let userID = UserManager.shared.currentUser?.id,
              let accountLease = AccountLeaseRuntime.shared.capture(forSubjectId: userID),
              ...
              AccountLeaseRuntime.shared.validate(accountLease, at: .request).allowed else {
            guard retryCount < 20 else {
                OwnerTruthInterviewBoundaryUIQASmoke.writeFailure("accountLeaseUnavailable")
                return
            }
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.2) { [weak self] in
                self?.runOwnerTruthInterviewBoundarySmoke(retryCount: retryCount + 1)
            }
            return
        }
        guard let keyWindow = UIApplication.shared.connectedScenes
            ...
            .first(where: { $0.isKeyWindow }),
              keyWindow.rootViewController is WarmTabBarController else {
            guard retryCount < 20 else {
                OwnerTruthInterviewBoundaryUIQASmoke.writeFailure("mainRootUnavailable")
                return
            }
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.2) { [weak self] in
                self?.runOwnerTruthInterviewBoundarySmoke(retryCount: retryCount + 1)
        ...
        keyWindow.rootViewController = navigationController
        keyWindow.makeKeyAndVisible()
        OwnerTruthInterviewBoundaryUIQASmoke.start(
            accountLease: accountLease,
            navigationController: navigationController
        )
        print("[UI_QA] OwnerTruthInterviewBoundarySmoke started")
    }
    ...
    func runOwnerTruthMemorySearchPresentationSmoke(retryCount: Int = 0) {
        guard let userID = UserManager.shared.currentUser?.id,
              let accountLease = AccountLeaseRuntime.shared.capture(forSubjectId: userID),
              accountLease.subjectId == userID,
              AccountLeaseRuntime.shared.validate(accountLease, at: .request).allowed else {
            guard retryCount < 20 else {
                QAScenarioResultWriter.writeAndLog(
                    ...
                    fileName: "owner-truth-memory-search-presentation-smoke-result.json",
                    smokeName: "OwnerTruthMemorySearchPresentationSmoke"
                )
                return
            }
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.2) { [weak self] in
                self?.runOwnerTruthMemorySearchPresentationSmoke(retryCount: retryCount + 1)
            }
            return
        }
        guard let keyWindow = UIApplication.shared.connectedScenes
        ...
        keyWindow.rootViewController = navigationController
        keyWindow.makeKeyAndVisible()
        OwnerTruthMemorySearchPresentationUIQASmoke.run(
            accountLease: accountLease,
            navigationController: navigationController
        ) { result in
            QAScenarioResultWriter.writeAndLog(
