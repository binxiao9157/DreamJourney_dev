...
require(echo.contains("hasTencentDigitalHumanProviderSpeechInFlight ||") && echo.contains("shouldResumeDialogAfterTencentDigitalHumanSpeech"), "manual interrupt decision should cover both active provider speech and the post-TextOver resume handoff")
...
require(stopBody.contains("interruptDigitalHumanPlayback(reason: \"userStop\")"), "manual stop should interrupt Tencent provider speech without closing the provider view")
...
require(textOverCompletionBody.contains("completedReplyText"), "Tencent TextOver completion must preserve reply text for speech-duration based resume delay")
require(textOverCompletionBody.contains("tencentDigitalHumanDialogResumeDelay(for: completedReplyText)"), "Tencent TextOver completion must wait based on estimated provider speech duration")
...
if let repliedStart = renderBody.range(of: "case .replied:")?.lowerBound,
   let errorStart = renderBody.range(of: "case .error", options: [], range: repliedStart..<renderBody.endIndex)?.lowerBound {
    ...
    require(repliedBody.contains("shouldResumeDialogAfterTencentDigitalHumanSpeech"), "Auto-resume replied UI should distinguish the short handoff state")
...
require(micTappedBody.contains("case .replied where shouldResumeDialogAfterTencentDigitalHumanSpeech"), "Tapping during Tencent auto-resume handoff should stop the conversation instead of looking like a fresh start")
