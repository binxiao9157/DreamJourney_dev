...
grep -Eq '"timeLetterDraftDeliveryPolicyPersisted"[[:space:]]*:[[:space:]]*true' "$RESULT_FILE" || fail "Time-letter draft should persist the non-delivery policy."
grep -Eq '"timeLetterSealedDeliveryPolicyPersisted"[[:space:]]*:[[:space:]]*true' "$RESULT_FILE" || fail "Time-letter sealed state should persist the non-delivery policy."
grep -Eq '"timeLetterBackendPayloadNonDelivering"[[:space:]]*:[[:space:]]*true' "$RESULT_FILE" || fail "Time-letter backend payload should remain non-delivering."
...
grep -Eq '"timeLetterDeliveryPolicyVisible"[[:space:]]*:[[:space:]]*true' "$RESULT_FILE" || fail "Time-letter delivery policy boundary should be visible."
