...
let script = read("tmp/visual-qa/prd-stitch-ui/run-archive-to-echo-smoke.sh")
...
assertContains(script, "xcrun simctl install", "smoke script installs the built app")
...
assertContains(script, "xcrun simctl get_app_container", "smoke script locates app data container")
