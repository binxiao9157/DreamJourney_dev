...
assertContains(echo, "lastDigitalHumanReplyTextSent", "Echo should dedupe digital human reply text across streaming prewarm and TTS fallback")
...
assertContains(echo, "skipped streaming prewarm; waiting for complete TTS sentence text", "Tencent-owned digital human speech should wait for complete sentence text instead of sending partial streaming fragments")
...
assertContains(echo, "pendingTencentDigitalHumanReplyText", "Echo should track provider-owned speech until Tencent TextOver")
assertContains(echo, "activeTencentDigitalHumanRequestID", "Echo should track Tencent stream text ReqId until provider TextOver")
assertContains(echo, "currentEchoTurnID", "Echo should keep a per-turn trace id for Tencent digital-human logs")
...
assertContains(echo, "activeTencentDigitalHumanRequestID == requestID", "Tencent TextOver timeout should only recover the still-active request")
...
assertContains(echo, "digitalHumanLivePanelView?.removeHostedProviderView()", "Tencent fallback should remove the provider view after SDK failure")
...
assertContains(echo, "without DialogEngine auto-resume", "Echo should return to idle after Tencent TextOver instead of reopening Fire/Volcengine capture")
...
assertContains(echo, "self.activeTencentDigitalHumanRequestID != nil || self.pendingTencentDigitalHumanReplyText != nil", "Echo should interrupt Tencent provider speech on a new user turn whenever a stream ReqId is still active")
...
assertContains(panelView, "setProviderModeEnabled(false)", "Panel should re-enable the local HTML renderer after provider fallback")
